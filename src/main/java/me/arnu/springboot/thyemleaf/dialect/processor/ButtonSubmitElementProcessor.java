...
public class ButtonSubmitElementProcessor extends AbstractElementTagProcessor {
    ...
    public ButtonSubmitElementProcessor(String dialectPrefix) {
        super(
                // 模板类型为HTML
                TemplateMode.HTML,
                // 标签方言前缀
...
//        String tableName = iProcessableElementTag.getAttributeValue("tableName");
        ...
        String[] submitV = vs[0].split("\\|");
        String submit = submitV[0];
        String submitName = submitV[1];
        StringBuffer result = new StringBuffer("<div class=\"layui-form-item text-center model-form-footer\">" +
                " <button class=\"layui-btn\" lay-filter=\"submitForm\" lay-submit>" + submitName + "</button>");
        if (vs.length > 1) {
            String[] closeV = vs[1].split("\\|");
            String close = closeV[0];
            String closeName = closeV[1];

            result.append(" <button class=\"layui-btn layui-btn-primary\" " + " type=\"button\" ew-event=\"closeDialog\">")
                    .append(closeName).append("</button></div>");
        }
