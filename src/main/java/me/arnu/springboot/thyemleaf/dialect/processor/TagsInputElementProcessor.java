...
public class TagsInputElementProcessor extends AbstractElementTagProcessor {
    ...
    private static final String TEMPLATE = "<input name=\"$ID$\" id=\"$ID$\" value=\"\" class=\"layui-hide\" lay-verType=\"tips\" " +
            REQUIRED + "><script th:inline=\"javascript\" type=\"text/javascript\">layui.use(['tagsInput'], function(){\n" +
    ...
    public TagsInputElementProcessor(String dialectPrefix) {
        super(
                // 模板类型为HTML
                TemplateMode.HTML,
                // 标签方言前缀
...
//        //select的name属性
        ...
        String id = vs[0];
        String isRequired = vs[1];
        String type = vs[2];
        String data = iProcessableElementTag.getAttributeValue("data");
        String result = TEMPLATE.replaceAll("\\$ID\\$", id).replaceAll("\\$DATA\\$", data);
        result = result.replaceAll(REQUIRED, isRequired.equals("1") ? REQUIRED_STR : "");
...
//        StringBuffer result = new StringBuffer("<a href=\"javascript:\" class=\"layui-btn btnOption  layui-btn-small btnAdd\" id=\"add\" data-param='[]' lay-event=\"add\"><i class=\"layui-icon layui-icon-add-1\"></i>" + name + "</a>");
